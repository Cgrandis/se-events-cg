...
export function GoogleSignInButton({ onClick, isLoading }: GoogleSignInButtonProps) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={isLoading}
      className={`flex items-center justify-center gap-2 bg-white border border-gray-300 rounded-md p-0.5 pr-3 mt-2 w-full h-12
        ${isLoading ? "opacity-50 cursor-not-allowed" : "hover:shadow-md transition"}`}
    >
